...
The order matters because of what this plugin records. A recorder streaming into a container stamps no length into a segment it has not finished, so the plugin's own recordings are exactly the files the headers cannot answer for, and they are also the long ones. Answering them with a decode would put gigabytes of PCM behind the cheapest question in the plugin, every time a transcribe dialog opens; the media read costs no samples at all. The enhanced player resolves its seek bar with the same manoeuvre through `DurationProbe`, which shares the far-seek constant rather than keeping a second copy of it.
