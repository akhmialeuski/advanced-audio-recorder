...
| Settings          | `settings/` (`settingsSchema`, `settingsDefinitions`, `SettingsTab`, `legacySettingsRenderer`, `settingsRenderMode`, `profileKinds`, `sections/`) | Settings model and defaults (`settingsSchema`), serialization, validation, the tab described as definitions (`settingsDefinitions`), the kinds of saved profile it builds every catalogue from (`profileKinds`), the pre-1.13 renderer for that same tree (`legacySettingsRenderer`), and the render path picked per Obsidian version (`settingsRenderMode`) |
