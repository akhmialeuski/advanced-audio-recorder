...
- **Commands come from one place**, the shared action registry, which resolves each list's context afresh on every palette check and hides a command whose context does not resolve. The session list runs against the recorder, where `Start/stop recording` and `Pause/resume recording` are always offered, the marker commands (`Add marker/chapter at current position` and the kind-fixed `Add bookmark at current recording position` and `Add chapter at current recording position`) need markers enabled and a live session, and `Select audio input device` is hidden where the platform cannot pick one. The file list runs against the active audio file (`Audio file info`, `Convert audio format`, `Split audio into parts`, `Clean up audio`, `Transcribe audio`, `Rename speakers`, `Generate chapters from transcript`, `Delete recording`), each with its own settings gate on top. The playback list runs against the active playback snapshot and covers transport, the two speed steps, the two chapter jumps, and the two marker actions, all absent while nothing plays. No default hotkeys are assigned - see [Recording](recording.md), [Audio player](audio-player.md#playback-commands-and-hotkeys), and [Settings reference](settings-reference.md).
