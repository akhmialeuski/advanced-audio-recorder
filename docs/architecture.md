...
- **Actions are defined once, in a shared action registry** (`src/actions/`). The same declarative list of file actions - info, convert, split, clean up, transcribe, rename speakers, generate chapters, delete - is rendered into the file and editor context menus and registered as palette commands, so every action can also be assigned a hotkey. The registry carries the recording-session marker actions and the playback actions the same way, the latter gated on the active playback snapshot rather than on a file. See [File operations](file-operations.md) and [Audio player](audio-player.md#playback-commands-and-hotkeys).
...
- **Commands come from two places.** Four session commands are registered directly (`Start/stop recording`, `Pause/resume recording`, `Add marker/chapter at current position` - gated on markers being enabled and a session being active - and `Select audio input device`), and the shared action registry adds the rest: `Add bookmark at current recording position` and `Add chapter at current recording position` (same recording gate), one command per file action (`Audio file info`, `Convert audio format`, `Split audio into parts`, `Clean up audio`, `Transcribe audio`, `Rename speakers`, `Generate chapters from transcript`, `Delete recording`), each gated on the active file being audio, and eleven playback commands (transport, speed steps, chapter jumps, and the two marker actions) gated on the registry publishing an active playback snapshot. No default hotkeys are assigned - see [Recording](recording.md), [Audio player](audio-player.md#playback-commands-and-hotkeys), and [Settings reference](settings-reference.md).
