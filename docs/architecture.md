...
The services the plugin talks to are declared once, in `src/providers/providers.ts`, and the settings follow that shape. A provider is one account with one endpoint and one key, and some of them answer more than one kind of request: Gemini and OpenAI both transcribe audio and run the post-processing prompts, Deepgram only transcribes, Anthropic only writes, and the local engine has neither endpoint nor key. Each provider therefore has a page of its own under the Engines entry, holding that connection and the catalogues it serves, while the transcription block and the post-processing block keep only the choice of which provider does the work. Before this, a provider that did both had its key on two rows and two endpoints for one account, and switching the post-processing vendor rewrote a shared base URL.
