...
The tab does not fit the declarative control model, because its device dropdowns come from live enumeration, its format options wait on an asynchronous encoder probe, and its per-track rows are built from a Map. It therefore returns one render definition and keeps drawing imperatively inside it. The row element of that definition is the single host the framework preserves, so the whole body lives there, the individual setting names travel on the definition as search aliases, and `styles/styles.css` strips that row's own flex layout, padding, background, and divider so the settings nested in it read as an ordinary settings column.

The version split itself lives in `settingsRenderMode.ts` as two objects behind one interface, and `AudioRecorderSettingTab` picks one when it is constructed by probing for `SettingTab.update()`, the method 1.13 added alongside the definition-driven render. The declarative mode returns the definition and re-renders by asking the framework, which reruns the definition itself, while the imperative mode returns an empty definition list, which is what makes a 1.13 host fall back to `display()`, and re-renders by rebuilding the container. Either way the previous body is released before it is replaced, on 1.13 through the cleanup callback the render definition hands back to the framework and below it by the mode itself, so a diagnostics test recording is never left reporting into a body that has already been dropped.
...
| Settings          | `settings/` (`settingsSchema`, `SettingsTab`, `settingsRenderMode`, `sections/`) | Settings model and defaults (`settingsSchema`), serialization, validation, the rendered settings tab, and the render path picked per Obsidian version (`settingsRenderMode`) |
