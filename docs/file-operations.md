...
- **Bitrate** offers `24`, `32`, `48`, `64`, `96`, `128`, `160`, `192`, `256`, and `320` kbps, cut to what the chosen target format can write: picking `MP3` drops everything below `32`, and selecting `WAV` hides the row entirely. The values below `64` are a mono speech mode - see [Bitrate guidance](formats.md#bitrate-guidance).
