...
- A mono **Channels** option downmixes inside the same pipeline: the mix uses the standard stereo-to-mono rules, and the left/right options keep one channel untouched. A right-channel pick on a file that is already mono falls back to the only channel instead of producing silence.
