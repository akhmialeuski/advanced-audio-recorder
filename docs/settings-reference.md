...
Every service the plugin talks to is configured once, on its own page under the **Engines** entry: its base URL, its API key, and the model catalogues it serves, one for speech and one for text where a provider does both. The transcription section and the LLM post-processing section keep only the choice of which provider does the work, so a key that serves both is entered in one place. Inside a catalogue the picking happens the same way everywhere: tap a row and it becomes the one in use, marked **In use**. The list also carries a filter once it is long, a delete button on each row, and an add button that asks for the id your endpoint serves. Adding a model selects it too, and deleting the one in use moves the selection to the first remaining id.
