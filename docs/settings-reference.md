...
Sub-section inside Transcription, below the engine fields. An experimental mode that transcribes each recording twice: LLM agents mine the first draft for the meeting's proper names, jargon, and English terms and acronyms, and the second pass re-decodes the audio biased toward them, with its language pinned to the first pass's detection. **Roughly 2x the engine cost and time plus several LLM calls per file** (on the provider configured under **LLM post-processing**, whose fields stay visible while this mode is on). Best-effort: any failure, and a second pass shorter than the safeguard ratio, keep the first pass's transcript. See [Advanced two-pass transcription](transcription.md#advanced-two-pass-transcription).

| Setting                                             | What it does                                                                                                                                                              | Options / range           | Default   |
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | --------- |
| **Advanced two-pass transcription (experimental)**  | Master toggle; its description carries the cost warning. Reveals the fields below.                                                                                        | On / Off                  | Off       |
| **Domain glossary**                                 | Optional names and terms, one per line, offered to the context agents as bias candidates; only terms the first draft gives evidence for are used.                         | Free text (one per line)  | - (empty) |
