...
| **Dictionary profiles**        | Named custom-dictionary glossaries (add, edit, remove). Pick one, or None, per run in the Transcribe dialog; the last pick is remembered for transcribe-on-save. Long lists trimmed to the engine limit; dropped terms reported.         | Profiles editor (name + terms)                                                           | None        |
...
Sub-section inside Transcription, below the Dictionary profiles. It is the advanced mode of the same dictionary biasing, so it reuses the terms from the selected Dictionary profile as its context candidates rather than a separate glossary. An experimental mode that transcribes each recording twice: LLM agents mine the first draft for the meeting's proper names, jargon, and English terms and acronyms, and the second pass re-decodes the audio biased toward them, with its language pinned to the first pass's detection. **Roughly 2x the engine cost and time plus several LLM calls per file** (on the provider configured under **LLM post-processing**, whose fields stay visible while this mode is on). Best-effort: any failure, and a second pass shorter than the safeguard ratio, keep the first pass's transcript. See [Advanced two-pass transcription](transcription.md#advanced-two-pass-transcription).

| Setting                                            | What it does                                                                                                                                                            | Options / range       | Default |
| -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------- |
| **Advanced two-pass transcription (experimental)** | Master toggle; its description carries the cost warning. Reveals the length safeguard below.                                                                            | On / Off              | Off     |
...
| **Chapter guidance profiles**    | Named prompts for how to divide the recording; the selected one is appended to the fixed base prompt. Add, edit, remove, and pick the profile per case. | Profiles        | Default |
