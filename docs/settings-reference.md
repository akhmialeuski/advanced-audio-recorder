...
| **Transcription dictionary**   | One term per line (names, abbreviations, domain terms) injected into each request to bias recognition. Every current engine supports it; over-long lists are trimmed to the engine limit and the dropped terms are reported.             | Multi-line text                                                                          | Empty       |
