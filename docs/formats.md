...
The encoder is asked about the bitrate only for a recording it will actually encode, which means a merged multi-track session or an offline-only format such as MP3, FLAC, or AAC. A single-track recording in a format MediaRecorder writes directly is encoded by the browser itself, which accepts the bitrate it is given, so nothing is substituted there and no notice appears.
