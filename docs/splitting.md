...
| **Bitrate**               | Bitrate used when re-encoding parts of a compressed source. **Hidden for WAV sources**, which are split without re-encoding.                                                   | `24` to `320` kbps, cut to what the source format writes          | Your **Audio bitrate** setting (`128`)                                 |
