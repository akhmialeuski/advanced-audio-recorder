...
- **Record WAV past the format's own ceiling.** A WAV file cannot describe more than 4 GB, which at 48 kHz stereo 16-bit is roughly six hours, and proportionally less at a wider [bit depth](formats.md#bit-depth). A notice warns at 90 percent of that, and a session that reaches the ceiling is refused at the moment you stop. Automatic splitting keeps every part well inside the limit, so it is what makes a WAV session of that length possible at all. See [Audio formats](formats.md#choosing-a-format).
