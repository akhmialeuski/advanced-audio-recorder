...
Works on desktop and mobile. Some features are desktop-only (multi-track recording, automatic splitting, local whisper.cpp transcription, input device selection); see [Recording on mobile](docs/recording.md#recording-on-mobile) for the platform differences. Requires Obsidian 1.6.6 or newer. MIT licensed.
