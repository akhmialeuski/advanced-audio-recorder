...
export class AudioRecorderSettingTab extends PluginSettingTab {
	...
	/**
	 * Renders the transcription settings. The master toggle reveals the
	 * provider, output, formatting, and LLM post-processing options.
	 * @param containerEl - The settings container element
	 */
	private renderTranscriptionSettings(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				'Transcribe recordings to text (speech-to-text), with optional speaker labels and LLM post-processing.',
			)
			.addToggle((toggle) =>
				toggle.setValue(s.transcriptionEnabled).onChange(async (v) => {
					s.transcriptionEnabled = v;
					await this.plugin.saveSettings();
					this.display();
				}),
			);
		...
		new Setting(containerEl)
			.setName('Transcribe after recording')
			.setDesc(
				'Automatically transcribe each recording once it is saved.',
			)
			.addToggle((toggle) =>
				toggle.setValue(s.transcribeOnSave).onChange(async (v) => {
					s.transcribeOnSave = v;
					await this.plugin.saveSettings();
				}),
			);
		...
		new Setting(containerEl)
			.setName('Engine')
			.setDesc(
				'Whisper API (cloud) or a local whisper.cpp binary (desktop).',
			)
			.addDropdown((dropdown) =>
				dropdown
					.addOption('whisper-api', 'Whisper API (OpenAI-compatible)')
					.addOption('local-whisper', 'Local whisper.cpp (desktop)')
					.setValue(s.transcriptionProvider)
					.onChange(async (v) => {
						s.transcriptionProvider =
							v as typeof s.transcriptionProvider;
						await this.plugin.saveSettings();
						this.display();
					}),
			);
		...
		new Setting(containerEl)
			.setName('Language')
			.setDesc('ISO code (e.g. en, ru, es) or "auto" to detect.')
			.addText((text) =>
				text.setValue(s.transcriptionLanguage).onChange((v) => {
					s.transcriptionLanguage = v.trim() || 'auto';
					this.saveTextSettingDebounced();
				}),
			);
		...
		new Setting(containerEl)
			...
			.setDesc(
				'Request speaker labels. Only effective with providers that support diarization.',
			)
			.addToggle((toggle) =>
				toggle.setValue(s.transcriptionDiarize).onChange(async (v) => {
					s.transcriptionDiarize = v;
					await this.plugin.saveSettings();
				}),
			);
		...
		if (s.transcriptionProvider === 'whisper-api') {
			...
			this.addTextSetting(
				...
				() => s.whisperApiModel,
				(v) => (s.whisperApiModel = v),
			);
		} else {
			this.addTextSetting(
				containerEl,
				'whisper.cpp binary path',
				'Absolute path to the whisper.cpp executable.',
				() => s.localWhisperBinaryPath,
				(v) => (s.localWhisperBinaryPath = v),
			);
			this.addTextSetting(
				containerEl,
				'Model path',
				'Absolute path to the whisper model file (.bin).',
				() => s.localWhisperModelPath,
				(v) => (s.localWhisperModelPath = v),
			);
			this.addTextSetting(
				containerEl,
				'Extra arguments',
				'Optional extra CLI arguments, space-separated.',
				() => s.localWhisperExtraArgs,
				(v) => (s.localWhisperExtraArgs = v),
			);
		}
	...
	/**
	 * Renders transcript output destination and formatting settings.
	 * @param containerEl - The settings container element
	 */
	private renderTranscriptOutputSettings(containerEl: HTMLElement): void {
		const s = this.plugin.settings;
		new Setting(containerEl).setName('Transcript output').setHeading();

		new Setting(containerEl)
			.setName('Destination')
			.setDesc('Insert into the note, save as a sidecar file, or both.')
			.addDropdown((dropdown) =>
				dropdown
					.addOption('note', 'Insert into note')
					.addOption('file', 'Save to file')
					.addOption('both', 'Note and file')
					.setValue(s.transcriptDestination)
					.onChange(async (v) => {
						s.transcriptDestination =
							v as typeof s.transcriptDestination;
						await this.plugin.saveSettings();
						this.display();
					}),
			);
		...
		if (
			...
			new Setting(containerEl)
				.setName('File format')
				.setDesc('Format for the transcript sidecar file.')
				.addDropdown((dropdown) =>
					dropdown
						.addOption('json', 'JSON (full data + speakers)')
						.addOption('srt', 'SubRip (.srt)')
						.addOption('vtt', 'WebVTT (.vtt)')
						.addOption('txt', 'Plain text (.txt)')
						.setValue(s.transcriptFileFormat)
						.onChange(async (v) => {
							s.transcriptFileFormat =
								v as typeof s.transcriptFileFormat;
							await this.plugin.saveSettings();
						}),
				);
		}
		...
		new Setting(containerEl)
			.setName('Note heading')
			.setDesc('Heading inserted above the transcript (empty for none).')
			.addText((text) =>
				text.setValue(s.transcriptHeading).onChange((v) => {
					s.transcriptHeading = v;
					this.saveTextSettingDebounced();
				}),
			);
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(s.transcriptIncludeTimestamps)
					.onChange(async (v) => {
						s.transcriptIncludeTimestamps = v;
						await this.plugin.saveSettings();
					}),
			);

		new Setting(containerEl)
			.setName('Timestamps as player links')
			.setDesc(
				'Render each timestamp as a #t= link that jumps the enhanced player.',
			)
			.addToggle((toggle) =>
				toggle
					.setValue(s.transcriptTimestampLinks)
					.onChange(async (v) => {
						s.transcriptTimestampLinks = v;
						await this.plugin.saveSettings();
					}),
			);

		new Setting(containerEl)
			.setName('Include speakers')
			.addToggle((toggle) =>
				toggle
					.setValue(s.transcriptIncludeSpeakers)
					.onChange(async (v) => {
						s.transcriptIncludeSpeakers = v;
						await this.plugin.saveSettings();
					}),
			);
		...
		this.addTextSetting(
			containerEl,
			'Timestamp format',
			'Template for the timestamp; {time} is the timecode/link.',
			() => s.transcriptTimestampFormat,
			(v) => (s.transcriptTimestampFormat = v || '[{time}]'),
		);
		this.addTextSetting(
			containerEl,
			'Speaker format',
			'Template for the speaker label; {speaker} is the name.',
			() => s.transcriptSpeakerFormat,
			(v) => (s.transcriptSpeakerFormat = v || '**{speaker}**'),
		);
		this.addTextSetting(
			containerEl,
			'Line format',
			'Arrangement of {timestamp} {speaker} {text}.',
			() => s.transcriptLineFormat,
			(v) =>
	...
	/**
	 * Renders the LLM post-processing settings.
	 * @param containerEl - The settings container element
	 */
	private renderLlmSettings(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				'Clean up punctuation/formatting or summarize the transcript with an LLM.',
			)
			.addToggle((toggle) =>
				toggle.setValue(s.llmPostProcessEnabled).onChange(async (v) => {
					s.llmPostProcessEnabled = v;
					await this.plugin.saveSettings();
					this.display();
				}),
			);
		...
		new Setting(containerEl).setName('Task').addDropdown((dropdown) =>
			dropdown
				.addOption('cleanup', 'Clean up (punctuation, formatting)')
				.addOption('summary', 'Summarize (key points + actions)')
				.addOption('custom', 'Custom instruction')
				.setValue(s.llmPostProcessTask)
				.onChange(async (v) => {
		...
		if (s.llmPostProcessTask === 'custom') {
			this.addTextSetting(
				containerEl,
				'Custom instruction',
				'System instruction applied to the transcript text.',
				() => s.llmCustomInstruction,
				(v) => (s.llmCustomInstruction = v),
			);
		}
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) =>
				dropdown
					.addOption('openai-compatible', 'OpenAI / Groq / Ollama')
					.addOption('anthropic', 'Anthropic (Claude)')
					.setValue(s.llmProvider)
					.onChange(async (v) => {
						s.llmProvider = v as typeof s.llmProvider;
						await this.plugin.saveSettings();
					}),
			);
		this.addTextSetting(
			containerEl,
			'LLM base URL',
			'API base URL (e.g. https://api.openai.com/v1, http://localhost:11434/v1, or https://api.anthropic.com/v1).',
			() => s.llmBaseUrl,
			(v) => (s.llmBaseUrl = v),
		);
		this.addTextSetting(
			containerEl,
			'LLM API key',
			'Leave empty for a local Ollama server. Stored in plugin data on this device.',
			() => s.llmApiKey,
			(v) => (s.llmApiKey = v),
			true,
		);
		this.addTextSetting(
			containerEl,
			'LLM model',
			'Model id (e.g. gpt-4o-mini, llama3.1, claude-opus-4-8).',
			() => s.llmModel,
			(v) => (s.llmModel = v),
		);
	}
	...
	/**
	 * Adds a debounced text setting bound to a getter/setter, optionally
	 * rendered as a password field for secrets.
	 * @param containerEl - Container
	 * @param name - Setting name
	 * @param desc - Setting description
	 * @param get - Reads the current value
	 * @param set - Writes the new value
	 * @param secret - Render as a password field
	 */
